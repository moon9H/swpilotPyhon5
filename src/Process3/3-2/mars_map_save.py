...
def draw_mars_map(area_map, area_struct, save_path='src/Process3/3-2/mars_map.png'):
    ...
    rock_coords = set(zip(rocks['x'], rocks['y']))
    ...
    # 화성 기지 (초록 삼각형)
    korea_base = area_struct[
    (area_struct['struct'] == 'Korea Mars Base') | (area_struct['struct'] == 'U.S. Mars Base Camp')
    ]
    ax.scatter(korea_base['x'], korea_base['y'], marker='^', s=400, color='green', edgecolors='black', label='화성 기지', zorder=5)
...
def main():
    ...
    draw_mars_map(area_map, area_struct)
